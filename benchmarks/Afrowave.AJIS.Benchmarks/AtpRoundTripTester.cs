...
""");
        ...
        var countries4Path = Path.Combine(solutionRoot, "test_data_legacy", "countries4.json");
        ...
        // Save ATP file
        var atpPath = Path.Combine(solutionRoot, "test_output", "countries4_roundtrip.atp");
        ...
        foreach (var attachment in attachments)
        {
            ...
            {
                var hash = sha256.ComputeHash(attachment.Data);
                var computed = BitConverter.ToString(hash).Replace("-", "").ToLower();
                
                var storedChecksum = attachment.Checksum?.ToLower() ?? "";
                bool isValid = computed == storedChecksum;

                var status = isValid ? "✅ VALID" : "❌ FAILED";
                var hashDisplay = computed.Substring(0, Math.Min(16, computed.Length));
        ...
        var allValid = checksumFailures == 0;
        var statusIcon = allValid ? "✅" : "❌";
        ...
        Console.WriteLine($"\n📊 STORAGE ANALYSIS:");
        var totalBinarySize = attachments.Sum(a => a.Data.Length);
        var atpFileSize = new FileInfo(atpPath).Length;
