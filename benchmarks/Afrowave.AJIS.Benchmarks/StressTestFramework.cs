...
public sealed class StressTestFramework
{
       ...
       string testFilePath)
   {
      ...
      try
      {
         ...
         {
            long localPeak = peakMemory;
            long localWorkingSetPeak = peakWorkingSet;
            while(sw.IsRunning)
            {
         ...
         // Run the operation
         object result = operation(testFilePath);
         ...
         int gcCollectionsAfter0 = GC.CollectionCount(0);
         int gcCollectionsAfter1 = GC.CollectionCount(1);
         int gcCollectionsAfter2 = GC.CollectionCount(2);
   ...
   private void PrintResult(StressTestResult result)
   {
      if(result.Success)
      {
         ...
         {
            double throughputMBps = result.FileSizeMB / (result.ElapsedMs / 1000.0);
