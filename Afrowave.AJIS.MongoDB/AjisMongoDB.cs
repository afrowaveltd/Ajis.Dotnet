...
/// </summary>
public class AjisBsonSerializer<T>(AjisConverter<T> converter) : SerializerBase<T> where T : notnull
{
   private readonly AjisConverter<T> _converter = converter;
...
/// </summary>
public class AjisMongoCollection<T>(IMongoDatabase database, string collectionName, string idField = "Id") where T : class
{
   private readonly IMongoCollection<BsonDocument> _collection = database.GetCollection<BsonDocument>(collectionName);
   private readonly AjisConverter<T> _converter = new();
   private readonly string _idField = idField;
   ...
   public async Task InsertAsync(T document)
   {
      ...
      var bsonDoc = new BsonDocument
        {
           { "data", ajisText },
           { "_id", BsonValue.Create(GetDocumentId(document)) }
        };
   ...
   private object GetDocumentId(T document)
   {
      ...
      return idProperty == null
         ...
         : idProperty.GetValue(document) ?? throw new InvalidOperationException("Document ID cannot be null");
   }
}
...
public static class AjisMongoExtensions
{
   ...
   public static void RegisterAjisSerializers()
   {
      ...
      BsonSerializer.RegisterSerializer(new AjisBsonSerializer<User>());
   }
...
/// </summary>
public class AjisMongoRepository<T>(IMongoDatabase database, string collectionName) where T : class
{
   private readonly AjisMongoCollection<T> _collection = new(database, collectionName);
