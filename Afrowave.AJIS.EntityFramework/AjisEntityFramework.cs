...
/// </summary>
public class AjisValueConverter<T>(AjisConverter<T> converter) : ValueConverter<T, string>(
    v => converter.Serialize(v),
    v => DeserializeOrThrow(converter, v)) where T : notnull
{
...
/// </summary>
public class AjisCollectionConverter<T>(AjisConverter<List<T>> converter) : ValueConverter<ICollection<T>, string>(
    v => converter.Serialize(v.ToList()),
    v => converter.Deserialize(v) ?? new List<T>()) where T : notnull
{
   private readonly AjisConverter<List<T>> _converter = converter;
...
/// </summary>
public class AjisFileRepository<T>(string filePath) where T : class, new()
{
   private readonly string _filePath = filePath;
   private readonly AjisConverter<List<T>> _converter = new();
