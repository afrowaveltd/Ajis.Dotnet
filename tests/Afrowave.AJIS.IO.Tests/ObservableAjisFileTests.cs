...
public class ObservableAjisFileTests : IDisposable
{
   ...
   {
      ObservableAjisFile<TestUser> observableFile = new ObservableAjisFile<TestUser>(_testFile);

      List<(TestUser User, ObservableAjisFile<TestUser>.ChangeType ChangeType)> events = new List<(TestUser User, ObservableAjisFile<TestUser>.ChangeType ChangeType)>();
      observableFile.Subscribe((user, changeType) => events.Add((user, changeType)));

      TestUser user = new TestUser { Id = 1, Name = "Alice" };
