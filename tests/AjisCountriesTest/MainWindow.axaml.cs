...
{
    private readonly ObservableCollection<Country> _countries = new();
    ...
    private void UpdateView(string search)
    {
        ...
        if (content == null || content.Children.Count < 3)
            return;
        ...
        var filtered = string.IsNullOrEmpty(search)
            ...
            : _countries.Where(c => 
                ...
                (c.Capital?.ToLower().Contains(search) ?? false) ||
                (c.Region?.ToLower().Contains(search) ?? false)).OrderBy(c => c.Name).ToList();
