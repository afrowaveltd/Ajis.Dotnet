...
public sealed class JsonToAjisConverter
{
       ...
       ConversionOptions? options = null)
   {
      ...
      try
      {
         ...
         if(detectBinary)
         {
            ...
            result.DetectedAttachments = [.. _detectedAttachments];
         }
         ...
         if(_detectedAttachments.Count > 0)
         {
            result.AtpMetadata = new()
            {
               ...
               BinaryAttachmentData = [.. _detectedAttachments]
            };
         }
       ...
       string outputFilePath)
   {
      ...
      AtpFileWrapper atpWrapper = new AtpFileWrapper
      {
         ...
         Attachments = conversionResult.DetectedAttachments ?? []
      };
      ...
      string atpJson = JsonSerializer.Serialize(atpWrapper, options);
   ...
   private JsonElement ProcessJsonForBinary(JsonElement element, string path)
   {
      switch(element.ValueKind)
      {
         case JsonValueKind.String:
            ...
            if(BinaryDetector.IsLikelyBinary(stringValue))
            {
   ...
   /// </summary>
   private static BinaryAttachment? CreateAttachmentFromString(string stringValue, string path)
   {
      try
      {
         byte[]? data = null;
         ...
         // Try base64
         if(BinaryDetector.TryDecodeBase64(stringValue, out var base64Data))
         {
            data = base64Data;
         }
         // Try hex
         else if(BinaryDetector.TryDecodeHex(stringValue, out var hexData))
         {
            data = hexData;
         }
   ...
   /// </summary>
   private static ImageType DetectImageType(byte[] imageData)
   {
   ...
   /// </summary>
   private static string SerializeToAjisText(JsonElement element) => JsonSerializer.Serialize(element, new JsonSerializerOptions
   {
      WriteIndented = true
   });
}
...
public sealed class BinaryDetector
{
   ...
   /// </summary>
   public static bool IsLikelyBinary(string value)
   {
   ...
   /// </summary>
   public static bool TryDecodeBase64(string value, out byte[]? result)
   {
      result = null;
      try
      {
   ...
   /// </summary>
   public static bool TryDecodeHex(string value, out byte[]? result)
   {
      result = null;
      try
      {
...
public sealed class AtpFileWrapper
{
   ...
   public List<(string Path, BinaryAttachment Attachment)> Attachments { get; set; } = [];
}
...
public sealed class AtpMetadata
{
   ...
   public List<(string Path, BinaryAttachment Attachment)> BinaryAttachmentData { get; set; } = [];
