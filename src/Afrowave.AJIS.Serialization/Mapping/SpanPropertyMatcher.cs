...
internal sealed class SpanPropertyMatcher
{
   ...
   public SpanPropertyMatcher(PropertyMetadata[] properties)
   {
      ...
      Dictionary<string, PropertyMetadata> exactMatchDict = new Dictionary<string, PropertyMetadata>(properties.Length);
   ...
   private static bool SpansEqualSimd(ReadOnlySpan<byte> a, byte[] b)
   {
      ...
      {
         Vector256<byte> vecA = Vector256.LoadUnsafe(ref Unsafe.AsRef(in a[i]));
         Vector256<byte> vecB = Vector256.LoadUnsafe(ref Unsafe.AsRef(in spanB[i]));
