...
public static class AjisGrouping
{
       ...
       Func<TKey, IEnumerable<T>, TResult> resultSelector) where T : notnull
   {
      ...
      Func<T, TKey> compiled = keySelector.Compile();
      IEnumerable<IGrouping<TKey, T>> groups = source.ToList().GroupBy(compiled);
       ...
       Expression<Func<T, TElement>> elementSelector) where T : notnull
   {
      ...
      Func<T, TKey> compiledKey = keySelector.Compile();
      Func<T, TElement> compiledElement = elementSelector.Compile();
