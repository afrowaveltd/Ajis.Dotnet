...
public class AjisSet<T>(IAjisDataSource<T> dataSource) : IAjisDataSource<T> where T : class
{
   private readonly IAjisDataSource<T> _dataSource = dataSource ?? throw new ArgumentNullException(nameof(dataSource));
