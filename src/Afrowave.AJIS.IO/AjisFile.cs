...
public static class AjisFile
{
   ...
   public static void Append<T>(string filePath, T item) where T : notnull
   {
      ...
      string newContent = content.TrimEnd()[..^1] + "," + serialized + "]";
   ...
   /// <summary>
   /// Creates an index for fast lookup by a key property.
   /// </summary>
   public static AjisFileIndex<T> CreateIndex<T>(string filePath, string keyProperty) where T : notnull
   {
      if(string.IsNullOrWhiteSpace(filePath))
         throw new ArgumentNullException(nameof(filePath));
      ...
      return new AjisFileIndex<T>(filePath, keyProperty);
   }

   /// <summary>
   /// Finds an object by key value using index.
   /// </summary>
   public static T? FindByKey<T>(string filePath, string keyProperty, object keyValue) where T : notnull
   {
      using var index = CreateIndex<T>(filePath, keyProperty);
      return index.FindByKey(keyValue);
   }

   /// <summary>
   /// Finds objects by predicate using index.
   /// </summary>
   public static IEnumerable<T> FindByPredicate<T>(string filePath, Func<T, bool> predicate) where T : notnull
   {
      return Enumerate<T>(filePath).Where(predicate);
   }

   /// <summary>
   /// Gets an object by key with Linq-like syntax.
   /// Usage: var user = AjisFile.Get&lt;User&gt;("users.json", "Name", "John");
   /// </summary>
   public static T? Get<T>(string filePath, string keyProperty, object keyValue) where T : notnull
   {
      return FindByKey<T>(filePath, keyProperty, keyValue);
   }

   /// <summary>
   /// Updates an object by key.
   /// </summary>
   public static void UpdateByKey<T>(string filePath, string keyProperty, object keyValue, Action<T> updateAction) where T : notnull
   {
      ...
      {
         updateAction(item);
         Create(filePath, items); // Rewrite entire file
      }
   }
   ...
   /// </summary>
   public static void DeleteByKey<T>(string filePath, string keyProperty, object keyValue) where T : notnull
   {
      var items = ReadAll<T>(filePath);
      items.RemoveAll(x => GetPropertyValue(x, keyProperty)?.Equals(keyValue) == true);
      Create(filePath, items); // Rewrite entire file
   }
      ...
      for(int i = 0; i < content.Length; i++)
      {
         ...
         else if(c == '}')
         {
            ...
            if(depth == 0)
            {
               ...
               string obj = content[start..++i].Trim();
               if(!string.IsNullOrWhiteSpace(obj))
                  objects.Add(obj);
