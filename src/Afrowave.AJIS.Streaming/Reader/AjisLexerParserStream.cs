...
public static class AjisLexerParserStream
{
   ...
   {
      using AjisStreamReader reader = new AjisStreamReader(stream, bufferSize);
      AjisLexerParser parser = new AjisLexerParser(reader, numberOptions, stringOptions, commentOptions, textMode, allowTrailingCommas, allowDirectives, preserveStringEscapes, emitDirectiveSegments, emitCommentSegments);
      return parser.Parse();
   }
   ...
   {
      using AjisStreamReader reader = new AjisStreamReader(stream, bufferSize);
      AjisLexerParserStreamingAsync parser = new AjisLexerParserStreamingAsync(
          reader,
          numberOptions,
          stringOptions,
          commentOptions,
