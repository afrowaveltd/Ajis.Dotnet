...
internal sealed class AjisParser
{
    ...
    public AjisValue Parse()
    {
        ...
        var lexer = new AjisLexer(_input.AsSpan(), _lexerOptions);
    ...
    private AjisValue ParseObject(ref AjisLexer lexer)
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                var key = DecodeStringToken(_input.AsSpan(_currentStart, _currentLength));
    ...
    {
        var rawText = _input.AsSpan(_currentStart, _currentLength);
        Advance(ref lexer);
    ...
    private string DecodeString(ReadOnlySpan<char> input)
    {
        ...
        // Use StringBuilder only when necessary
    ...
    {
        var text = _input.AsSpan(_currentStart, _currentLength);
        Advance(ref lexer);

        var hasSeparators = text.IndexOf('_') >= 0 || text.IndexOf(' ') >= 0;
        ...
        if (text.Length > 0 && text.IndexOf('_') < 0 && text.IndexOf(' ') < 0 &&
